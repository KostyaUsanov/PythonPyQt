...
class TrainView(View):
    def get(self, request):
        ...
        #max_entry_authors = Entry.objects.aggregate(max_entry_authors=Max('author'))
        #max_count_entry = Entry.objects.aggregate(max_count_entry=Max(Count('text')))
        self.answer2 = Author.objects.annotate(num_entries=Count('entries')).order_by('-num_entries')[:1]
        #self.answer2 = Entry.objects.filter(author=max_entry_authors['max_entry_authors'])     #TODO Какой автор имеет наибольшее количество опубликованных статей?
